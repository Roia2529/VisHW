...
class TileChart {
    ...
    update (electionResult, colorScale){
        ...
        //Use this tool tip element to handle any hover over the chart
            ...
            this.legendSvg.select(".legendQuantile")
                    .attr("transform", "translate(" + this.margin.left*3 +",50)")
                    .call(legendQuantile);
            ...
            let tileWidth = 120;
            let tileHeight= 100;
            let textlocX = tileWidth/2;
            let textlocY = tileHeight/2;
            ...
            tiles = tiles.enter().append('rect').merge(tiles);
                    tiles
                       .attr('x',(d)=>{
                            return d.Space*tileWidth+this.margin.left;
                       })
                       .attr('y',(d)=>{ return d.Row*tileHeight}) 
                       .attr('width',tileWidth) 
                       .attr('height',tileHeight)
                       .attr('fill',(d)=>{return colorScale(+d.RD_Difference)})
                       .classed('tile',true);

            //Display the state abbreviation and number of electoral votes on each of these rectangles
            this.svg.selectAll('.tilestext').remove();

            let tilesText = this.svg.selectAll('text')
                        .data(electionResult);
            tilesText.exit().remove();

            tilesText = tilesText.enter().append('text').merge(tilesText);
            
            tilesText.append('tspan')
                       .attr('x',(d)=>{ return d.Space*tileWidth+this.margin.left+textlocX; })
                       .attr('y',(d)=>{ return d.Row*tileHeight+textlocY;}) 
                       .classed('tilestext',true)
                       .text((d)=>{return d.Abbreviation;});
                       
            tilesText.append('tspan')
                       .attr('x',(d)=>{ return d.Space*tileWidth+this.margin.left+textlocX; })
                       .attr('y',(d)=>{ return d.Row*tileHeight+textlocY+25 ;}) 
                       .classed('tilestext',true)
